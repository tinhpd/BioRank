...
# from improved_pagerank.matrix_creation.matrix_aggregation import MatrixAggregation
...
# 		aggregated_graph = self._aggregate_adjacency_matrix(PPI_sub_normalized_sub_network,CO_expression_normalized_sub_network,nodes_from_given_policy)
...
# 	def _aggregate_adjacency_matrix(self,PPI_network,CO_expression_network, V):
...
# 				final_graph.add_edge(source, target, weight = self.beta*score_ppi_network)
...
# 				final_graph[source][target]['weight'] = final_graph[source][target]['weight'] + (1 - self.beta)*score_co_expression_network

# 			else:
# 				if source in V and target in V:
					
# 					if score_co_expression_network > 0.0:
# 						final_graph.add_edge(source, target, weight =  (1 - self.beta)*score_co_expression_network)
...
class ConvexCombinationMatrixAggregationCreation(MatrixAggregation):
    ...
    def choose_policy(self, V, U, chosen_policy = "PPI_network"):
        if chosen_policy == "Intersection":
            ...
            print("No corrected chosen policy", chosen_policy)
            exit(self.choose_policy_exit)
    ...
    def run(self, chosen_policy):
        ...
        # Chạy lại và sửa trọng số mới
        #self.invert_edge_weights(aggregated_graph)
    ...
    def invert_edge_weights(self, G):
        for edge in G.edges():
            current_weight = G[edge[0]][edge[1]]['weight']
            new_weight = 1.0 - current_weight
